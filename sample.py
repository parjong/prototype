...
class Page(BaseModel):
    title: str = Field(description="The title of concise main title of the article or page")
    date: str = Field(description="The issue or publication date as YYYY/MM/DD format (????/??/?? if unknown)")
...
def check(response) -> Page:
    ...
    structured_llm = ChatGoogleGenerativeAI(model="gemini-2.5-flash").with_structured_output(Page)

    chain = prompt | structured_llm

    return chain.invoke({"content": response.text})
...
for url in cases:
    resp, title = get(url)
    page = check(resp)
    data.append({"url": url, "bs4.title": title, "gemini.date": page.date, "gemini.title": page.title})
